...
export const Step5ConfigureErrors = ({ errorConfig, behaviorRules, onErrorChange, onBehaviorChange, getAffectedTables, getAffectedRecords, tables, getCurrentTableData, onNext, onPrevious }) => {
  ...
  /*
  React.useEffect(() => {
  // Auto-select first table with errors when slider changes
    const firstTableWithErrors = tables.find(table => {
      const hasErrors = Object.keys(errorConfig).some(key => {
        if (errorConfig[key] > 0) {
          const affectedTables = getAffectedTables(key);
          const affectedRecords = getAffectedRecords(key);
          return affectedTables.includes(table.name) && affectedRecords >= 1;
        }
        return false;
      });
      return hasErrors;
  ...
  }, [errorConfig, tables, getAffectedTables, getAffectedRecords]);
*/

/*
  React.useEffect(() => {
    // Only auto-select if no table is currently selected
    if (selectedErrorTable) return;
  ...
  }, [errorConfig, tables, getAffectedTables, getAffectedRecords, selectedErrorTable]);
*/

  React.useEffect(() => {
    // Only auto-select if no table is currently selected OR if current table has no errors
    const currentTableHasErrors = selectedErrorTable && Object.keys(errorConfig).some(key => 
    ...
    // Auto-select first table with errors
    const firstTableWithErrors = tables.find(table => 
  ...
  const errorCategories = [
    {
      ...
      errors: [
        ...
        { key: 'missingClientId', label: 'Missing Client / Policy Holder  ID', columns: ['policyholder_id'] },
        ...
        { key: 'inconsistentDateFormats', label: 'Inconsistent Date Formats', columns: ['effective_date', 'claim_date','payment_date'] },
    ...
    },
    {
      ...
      errors: [
        { key: 'overlappingPolicyPeriods', label: 'Overlapping Policy Periods', columns: ['expiry_date'] },
    ...
    },
    {
      ...
      errors: [
        { key: 'orphanedClaims', label: 'Orphaned Claims without Policy ID', columns: ['policy_id'] },
        { key: 'paymentsWithoutClaims', label: 'Payments Without Claims', columns: ['claim_id'] },
        { key: 'policyWithoutClient', label: 'Policy Without Valid Policy Holder / Client ID', columns: ['policyholder_id'] },
  ...
  /*
  const affectedTableData = React.useMemo(() => {
    return selectedErrorTable ? getCurrentTableData(selectedErrorTable, true) : null;
  }, [selectedErrorTable, errorConfig, refreshKey]);
  */
    ...
    <div className="step-container">
      ...
      <div className="card">
        ...
        <div className="table-selector">
          {tables.map(table => {
            const hasConfiguredErrors = Object.keys(errorConfig).some(key => 
              errorConfig[key] > 0 && getAffectedTables(key).includes(table.name) && getAffectedRecords(key) >= 1
            );
            return (
              <button
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                    <strong>Affected Records:</strong> {affectedRecords} records ({errorConfig[error.key]}% configured)
                  </div>
                </div>
                <div className="error-slider-container">
                  <input
                    ...
                    onChange={(e) => {
                      onErrorChange(error.key, e.target.value);
                    }}
                    onInput={(e) => {
                      onErrorChange(error.key, e.target.value);
                    }}
