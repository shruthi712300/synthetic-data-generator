saturamTileViewDivGeneric = function () {
    ...
    function Tile(intType, intName, execStatus, type, intId, selected) {
        console.log(intType, intName, execStatus, type, intId, selected);
    ...
    function render_integration_section(group_id, integration_type, subject_1, divId) {
    ...
    function render_destination_section(group_id, integration_type, subject_1, divId) {
        let _t = $('#' + group_id).children(' div.brand-card').clone();
        _t.find('img').attr("src", str1.concat(integration_type, ".jpg"));
    ...
    function render_appsource_section(group_id, integration_type, int_id, int_name, is_selected, divId) {
        console.log(divId, integration_type);
        let _t = $('#' + group_id).children(' div.brand-card').clone();
       ...
       $('#div_source').find(divId).append(_t);
        return _t;
    }

    function render(tileList, divId) {
        console.log(tileList);
        console.log(divId);
        $(divId).empty();
        $('#div_destination').find(divId).empty();
        $('#div_source').find(divId).empty();
        ...
        for (let i = 0; i < tileList.length; i++) {
            ...
            if (tile_type === "allsource" || tile_type === "All") {
                if (tile_integration_type === "postgres" || tile_integration_type === "ibmdb" || tile_integration_type === "kafka"
                    ...
                    render_integration_section('_dummy_0', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "mysql") {
                    render_integration_section('_dummy_mysql', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "kafka") {
                    render_integration_section('_dummy_kafka', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "osisoft") {
                    render_integration_section('_dummy_osisoft', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "excel") {
                    render_integration_section('_dummy_excel', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "s3") {
                    render_integration_section('_dummy_s3', tile_integration_type, "S3", divId);
                }
                else if (tile_integration_type === "googlebig") {
                    render_integration_section('_dummy_bq', tile_integration_type, "Google BigQuery", divId);
                }
                else if (tile_integration_type === "gcs") {
                    render_integration_section('_dummy_gcs', tile_integration_type, "Google BigQuery", divId);
                }
                else if (tile_integration_type === "twitter") {
                    render_integration_section('_dummy_twitter', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "facebook") {
                    render_integration_section('_dummy_facebook', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "instagram") {
                    render_integration_section('_dummy_instagram', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "googletrends") {
                    render_integration_section('_dummy_gt', tile_integration_type, "Google Trends", divId);
                }
                    ...
                    render_integration_section('_dummy_hadoop', tile_integration_type, tile_integration_type, divId);
                }
                else if (tile_integration_type === "sftp") {
                    render_integration_section('sftp_connector', tile_integration_type, tile_integration_type.toUpperCase(), divId);
                }
                else {
                    render_integration_section('_dummy', tile_integration_type, tile_integration_type, divId);
                }
            }
                ...
                if (tile_integration_type === "osisoft") {
                    ...
                    render_integration_section('kafka_connector', tile_integration_type, tile_integration_type, divId);
                }
            }
                ...
                if (tile_integration_type === "googlebig") {
                    render_destination_section('_dummy_googlebig', tile_integration_type, "Google BigQuery", divId);
                }
                else if (tile_integration_type === "postgres") {
                    render_destination_section('_dummy_postgres_dest', tile_integration_type, "Postgres", divId);
                }
                else if (tile_integration_type === "mysql") {
                    render_destination_section('_dummy_postgres_dest', tile_integration_type, "MySQL", divId);
                }
                else if (tile_integration_type === "teradata") {
                    render_destination_section('_dummy_teradata_dest', tile_integration_type, "Teradata", divId);
                }
                else if (tile_integration_type === "s3") {
                    render_destination_section('_dummy_s3_dest', tile_integration_type, "S3", divId);
                }
                else if (tile_integration_type === "gcs") {
                    render_destination_section('_dummy_gcs_dest', tile_integration_type, "GCS", divId);
                }else if (tile_integration_type === "adl") {
                    render_destination_section('_dummy_adl_dest', tile_integration_type, "Azure Data Lake", divId);
                }else if (tile_integration_type === "azure_blob") {
                    render_destination_section('_dummy_azure_blob_dest', tile_integration_type, "Azure Blob Storage", divId);
                }
                else {
                    render_destination_section('_dummy_dest', tile_integration_type, tile_integration_type, divId);
                }
            }
                ...
                $('#div_destination').find(divId).append(_t);
            }
            else if (tile_type === "appsource") {
                if (tile_integration_type === "twitter") {
                    render_appsource_section('_dummy_5', tile_integration_type, tile_info.intId, tile_info.intName, tile_info.selected, divId);
                }
                else if (tile_integration_type === "googletrends") {
                    render_appsource_section('_dummy_7', tile_integration_type, tile_info.intId, tile_info.intName, tile_info.selected, divId);
                }
                else if (tile_integration_type === "facebook") {
                    render_appsource_section('_dummy_6', tile_integration_type, tile_info.intId, tile_info.intName, tile_info.selected, divId);
                }
                else {
                    let _t = render_appsource_section('_dummy_4', tile_integration_type, tile_info.intId, tile_info.intName, tile_info.selected, divId);
